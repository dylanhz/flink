...
class OverAggregateITCase(mode: StateBackendMode, unboundedOverVersion: Int)
  ...
  def testBitmapBuildAgg(): Unit = {
    val sql =
      """
        |SELECT
        |  a, b,
        |  CAST(BITMAP_BUILD_AGG(a) OVER (ORDER BY proctime ROWS BETWEEN 4 PRECEDING AND CURRENT ROW) AS STRING),
        |  CAST(BITMAP_BUILD_AGG(CAST(b AS INT)) OVER (ORDER BY proctime ROWS BETWEEN 4 PRECEDING AND CURRENT ROW) AS STRING)
        |FROM MyTable
      """.stripMargin
  ...
  @TestTemplate
  def testBitmapAndOrXorAgg(): Unit = {
    val sql =
      """
        |SELECT
        |  a, c,
