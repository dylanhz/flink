...
class OverAggregateITCase extends BatchTestBase {
  ...
  def testBitmapBuildAgg(): Unit = {
    checkResult(
      "SELECT " +
        "d, e, " +
        "CAST(BITMAP_BUILD_AGG(d) OVER (ORDER BY e ROWS BETWEEN 4 PRECEDING AND CURRENT ROW) AS STRING), " +
        "CAST(BITMAP_BUILD_AGG(CAST(e AS INT)) OVER (ORDER BY e ROWS BETWEEN 4 PRECEDING AND CURRENT ROW) AS STRING) " +
        "FROM NullTable5",
      Seq(
        row(1, 1L, "{1}", "{1}"),
        row(2, 2L, "{1,2}", "{1,2}"),
